...
public struct AppleSearchBarNoteView: View {
    ...
    @AppStorage(PrefKey.notePrinterSoundEnabled) var notePrinterSoundEnabled: Bool = true
    @AppStorage(PrefKey.barModeRaw) var barModeRaw: String = "chat"
    ...
    @AppStorage(PrefKey.showEmotionPlayer) var showEmotionPlayer: Bool = false
    @AppStorage(PrefKey.isChatLockedInPlace) var isChatLockedInPlace: Bool = false
    @AppStorage(PrefKey.isChatDockedToMenuBar) var isChatDockedToMenuBar: Bool = false
    ...
    @AppStorage(PrefKey.chatOffsetX) var chatOffsetX: Double = 0.0
    @AppStorage(PrefKey.chatOffsetY) var chatOffsetY: Double = 0.0
    @State private var dragCurrentTranslation: CGSize = .zero
    @State private var activeSettingsTab: UnifiedSettingsTab = .miniDock
    @AppStorage(PrefKey.selectedNoteOutputDestination) var selectedOutputDestinationRaw: String = NoteOutputDestination.polaroidPng.rawValue
    @AppStorage(PrefKey.customNotesDestinationPath) var customNotesDestinationPath: String = ""
    ...
    @AppStorage(PrefKey.showMiniDockInChatBar) var showMiniDockInChatBar: Bool = true
    ...
    @AppStorage(PrefKey.chatSmartCleanMode) var chatSmartCleanMode: Bool = true
    @AppStorage(PrefKey.showAppsMiniMap) var showAppsMiniMap: Bool = false
    @AppStorage(PrefKey.isChatFullScreen) var isChatFullScreen: Bool = false
    ...
    public var body: some View {
        if isChatDockedToMenuBar {
            ...
            HStack(alignment: .top, spacing: 14) {
                VStack(spacing: 10) {
                    // ── Mini Dock & Battery Strip in Chat Bar (Moves smoothly down into place from top notch with bump effect) ──
                    if showMiniDockInChatBar {
                        chatBarMiniDockStrip
                            ...
                            .animation(.spring(response: 0.38, dampingFraction: 0.70), value: showMiniDockInChatBar)
                    }
                    ...
                    // ── Live Smart Settings Dropdown Strip (when typing "settings" or preferences queries) ──
                    if isSettingsQuery && currentMode != .settings {
                        smartSettingsDropdownStripView
                            .transition(.move(edge: .bottom).combined(with: .opacity))
                    }
            ...
            .background(
                ZStack {
                    ...
                    // Apple Glass Specular Top Glaze
                    LinearGradient(
                        stops: [
            ...
            .overlay(
                Capsule()
                    .strokeBorder(
                        LinearGradient(
                            stops: [
                                .init(color: Color.white.opacity(flashPulse ? 0.95 : (isFocused ? 0.55 : 0.32)), location: 0.0),
                                .init(color: Color.white.opacity(flashPulse ? 0.85 : (isFocused ? 0.25 : 0.14)), location: 0.40),
                                .init(color: Color.white.opacity(flashPulse ? 0.65 : (isFocused ? 0.12 : 0.05)), location: 1.0)
                            ],
                            startPoint: .top,
                            endPoint: .bottom
                        ),
                        lineWidth: flashPulse ? 1.4 : 0.85
                    )
            ...
            .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("NexusSelectSettingsTab"))) { notif in
                if let tab = notif.object as? UnifiedSettingsTab {
                    ...
                    withAnimation(.spring(response: 0.28, dampingFraction: 0.80)) {
                        barModeRaw = BarMode.settings.rawValue
                        if isChatDockedToMenuBar {
                            isChatDockedToMenuBar = false
                        }
                    }
        ...
        // ── Applications Mini-Map Drawer to the Right ("and the applicatons are a mini map to the right toggle on off") ──
        if showAppsMiniMap {
            compactRightSideAppsMiniMapView
                .transition(.asymmetric(
                    insertion: .scale(scale: 0.92).combined(with: .opacity).combined(with: .move(edge: .trailing)),
                    removal: .scale(scale: 0.92).combined(with: .opacity).combined(with: .move(edge: .trailing))
                ))
    ...
    private func handleSubmit() {
        ...
        if isSettingsQuery && currentMode != .terminal {
            openSmartSettings(for: trimmed)
            return
        }
    ...
    private func handleCommandSubmit() {
        if currentMode == .chat {
            ...
            let proModel: String = {
                if localModels.hasGeminiKey { return "gemini-2.0-flash" }
        ...
        DragGesture(minimumDistance: 1)
            ...
            .onEnded { val in
                if !isChatLockedInPlace {
                    ...
                    UserDefaults.standard.set(chatOffsetX, forKey: PrefKey.chatOffsetX)
                    UserDefaults.standard.set(chatOffsetY, forKey: PrefKey.chatOffsetY)
                    UserDefaults.standard.set(true, forKey: PrefKey.isChatDetached)
                }
            }
    }
    ...
    private var rightSideControlsGroup: some View {
        HStack(spacing: 5) {
            ...
            HStack(spacing: 2) {
                ...
                // 🗺️ Applications Mini-Map Toggle ("and the applicatons are a mini map to the right toggle on off")
                Button(action: {
                    withAnimation(.spring(response: 0.28, dampingFraction: 0.82)) {
                        showAppsMiniMap.toggle()
                    }
                    HapticFeedback.selection()
                }) {
                    Image(systemName: showAppsMiniMap ? "map.fill" : "map")
                        .font(.system(size: 10.5, weight: .bold))
                        .foregroundColor(showAppsMiniMap ? .orange : .white.opacity(0.50))
                        .padding(4)
                        .background(showAppsMiniMap ? Capsule().fill(Color.orange.opacity(0.25)) : Capsule().fill(Color.clear))
                }
                .buttonStyle(.plain)
                .help("Toggle Applications Mini-Map Radar 🗺️")
            }
            .padding(2)
                        ...
                        Section(provider.rawValue) {
                            ForEach(models) { model in
                                Button(action: {
                                    ...
                                    HStack {
                                        Text(model.displayName)
                                        if localModels.manualSelectedModel == model.id || localModels.effectiveModel == model.id {
                                            Text("✓")
                                        }
                                    }
                                }
    ...
    private var chatBarSpacesSegment: some View {
        ...
        .gesture(
            DragGesture(minimumDistance: 6)
                .onEnded { value in
                    let cur = MacDesktopsManager.shared.currentSpaceIndex
                    let total = MacDesktopsManager.shared.spaces.count
                    if value.translation.width < -10 {
                        let next = min(max(total, 1), cur + 1)
                        if next != cur {
                            HapticFeedback.selection()
    ...
    private func chatBarDockAppIcon(item: DockAppItem) -> some View {
        let isAppActive = item.runningApp?.isActive == true || (item.processIdentifier > 0 && item.processIdentifier == dockManager.activePid)
        Button {
            HapticFeedback.selection()
        ...
        .buttonStyle(.plain)
        .help(item.isRunning ? "\(item.name) (Running) — Click to bring to front" : "\(item.name) — Click to launch")
        .contextMenu {
    ...
    private var chatBarFilteredDockItems: [DockAppItem] {
        if dockActiveAppsOnly {
            return dockManager.dockItems.filter { $0.isRunning }
        }
        return dockManager.dockItems
    }
    ...
    private var chatBarAppsSegment: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 4) {
                ForEach(chatBarFilteredDockItems) { item in
    ...
    private var chatBarMiniDockStrip: some View {
        HStack(spacing: 6) {
            ...
            if dockShowFolderStacks {
                Button(action: {
                    HapticFeedback.selection()
                    let path = (UserDefaults.standard.string(forKey: PrefKey.barFolderPath) ?? NSHomeDirectory() + "/Downloads") as NSString
                ...
                .contextMenu {
                    Button("Open Downloads") {
                        NSWorkspace.shared.open(FileManager.default.urls(for: .downloadsDirectory, in: .userDomainMask).first ?? URL(fileURLWithPath: NSHomeDirectory()))
                    }
                    Button("Open Documents") {
                        NSWorkspace.shared.open(FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first ?? URL(fileURLWithPath: NSHomeDirectory()))
                    }
            ...
            if dockAlwaysShowTrash {
                Button(action: {
                    ...
                    ZStack(alignment: .topTrailing) {
                        Image(systemName: trashMonitor.isTrashFull ? "trash.fill" : "trash")
    ...
    // MARK: - 🗺️ Compact Right-Side Applications Mini-Map ("and the applicatons are a mini map to the right toggle on off")
    @State private var appsMiniMapFilter: String = ""

    @ViewBuilder
    private var compactRightSideAppsMiniMapView: some View {
        let currentAppModel = AppModel.shared ?? AppModel()
        let matching = currentAppModel.filteredApps(search: appsMiniMapFilter, category: nil)
        let runningCount = currentAppModel.runningApps.count
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 5) {
                ...
                TextField("Filter mini-map...", text: $appsMiniMapFilter)
                    .textFieldStyle(.plain)
                    .font(.system(size: 10))
                    .foregroundColor(.white)
                if !appsMiniMapFilter.isEmpty {
                    Button(action: { appsMiniMapFilter = "" }) {
                        Image(systemName: "xmark.circle.fill")
                            .font(.system(size: 9))
                            .foregroundColor(.white.opacity(0.4))
                    }
            ...
            // Scrollable 4-column Mini Icon Grid
            ScrollView(.vertical, showsIndicators: false) {
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(minimum: 44, maximum: 58), spacing: 6), count: 4), spacing: 8) {
                    ForEach(matching.prefix(36)) { app in
                        let isRunning = currentAppModel.runningApps.contains { running in
                            running.bundleURL == app.url || running.localizedName == app.name
                        }
                        Button(action: {
                            HapticFeedback.selection()
                            NSWorkspace.shared.openApplication(at: app.url, configuration: NSWorkspace.OpenConfiguration(), completionHandler: nil)
                            showFeedback("Launched \(app.name) 🚀")
                        }) {
                            VStack(spacing: 2) {
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 6, style: .continuous)
                                    .fill(isRunning ? Color.white.opacity(0.12) : Color.white.opacity(0.04))
                            )
                        }
                        .buttonStyle(.plain)
                        .help(app.name + (isRunning ? " (Running)" : ""))
                    }
                }
                .padding(.vertical, 4)
            }
            .frame(minWidth: 240, maxWidth: 260, minHeight: 200, maxHeight: 320)
        }
        .padding(12)
        .background(
            VisualEffectBlur(material: .hudWindow, blendingMode: .withinWindow, state: .active)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(LinearGradient(colors: [Color.orange.opacity(0.50), Color.white.opacity(0.15)], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 0.85)
                )
    ...
    // MARK: - Smart Settings Query Intelligence & Dropdown
    private var isSettingsQuery: Bool {
        let trimmed = noteText.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !trimmed.isEmpty else { return false }
        if trimmed.hasPrefix("settings") || trimmed.hasPrefix("setting") || trimmed.hasPrefix("!settings") ||
           trimmed.hasPrefix("prefs") || trimmed.hasPrefix("preferences") || trimmed.hasPrefix("!prefs") ||
           trimmed.hasPrefix("setup") || trimmed.hasPrefix("options") || trimmed.hasPrefix("config") ||
           trimmed == "genie settings" || trimmed == "genie" || trimmed == "gold gate settings" || trimmed == "goldgate" || trimmed == "gold gate" {
            return true
        }
        for tab in UnifiedSettingsTab.allCases {
            for kw in tab.keywords {
                if trimmed == kw || (trimmed.count >= 4 && (trimmed.contains(kw) || kw.contains(trimmed))) {
                    return true
                }
            }
        }
    ...
    private var matchingSettingsTabs: [UnifiedSettingsTab] {
        let trimmed = noteText.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !trimmed.isEmpty else { return UnifiedSettingsTab.allCases }

        if trimmed == "settings" || trimmed == "setting" || trimmed == "!settings" || trimmed == "prefs" || trimmed == "preferences" || trimmed == "setup" || trimmed == "options" {
            return UnifiedSettingsTab.allCases
        }

        var matches: [UnifiedSettingsTab] = []
        for tab in UnifiedSettingsTab.allCases {
            if tab.rawValue.lowercased().contains(trimmed) {
                matches.append(tab)
                continue
            }
            for kw in tab.keywords {
                if trimmed.contains(kw) || kw.contains(trimmed) {
                    if !matches.contains(tab) {
                        matches.append(tab)
                    }
                    break
                }
        ...
        return matches.isEmpty ? UnifiedSettingsTab.allCases : matches
    }

    private func detectTargetSettingsTab(query: String) -> UnifiedSettingsTab {
        let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if trimmed.contains("dock") || trimmed.contains("bar") || trimmed.contains("menu") || trimmed.contains("battery") || trimmed.contains("percentage") {
            return .miniDock
        } else if trimmed.contains("screen") || trimmed.contains("matrix") || trimmed.contains("size") || trimmed.contains("iphone") || trimmed.contains("ipad") || trimmed.contains("720p") || trimmed.contains("trickster") || trimmed.contains("compact") || trimmed.contains("split") {
            return .virtualScreen
        } else if trimmed.contains("app") || trimmed.contains("applications") || trimmed.contains("grid") || trimmed.contains("launcher") {
            return .applications
        } else if trimmed.contains("desktop") || trimmed.contains("file") || trimmed.contains("wallpaper") {
            return .desktop
        } else if trimmed.contains("sound") || trimmed.contains("audio") || trimmed.contains("smoke") || trimmed.contains("haptic") || trimmed.contains("effect") {
            return .soundAndSmoke
        } else if trimmed.contains("model") || trimmed.contains("ai") || trimmed.contains("ollama") || trimmed.contains("gemini") || trimmed.contains("claude") || trimmed.contains("gpt") || trimmed.contains("engine") {
            return .models
        } else if trimmed.contains("glass") || trimmed.contains("living") || trimmed.contains("vibrancy") || trimmed.contains("theme") {
            return .livingGlass
        } else if trimmed.contains("access") || trimmed.contains("system") || trimmed.contains("permission") || trimmed.contains("cgevent") || trimmed.contains("applescript") {
            return .systemAccess
        } else if trimmed.contains("hugging") || trimmed.contains("hf") || trimmed.contains("gguf") {
            return .huggingface
        } else if trimmed.contains("privacy") || trimmed.contains("general") || trimmed.contains("reset") {
            return .generalAndPrivacy
        }
        return .miniDock
    }

    private func openSmartSettings(for query: String) {
        let targetTab = detectTargetSettingsTab(query: query)
        ...
        AppDelegate.shared?.showApplicationsSettings(tab: targetTab)
        NotificationCenter.default.post(
            name: NSNotification.Name("NexusSelectSettingsTab"),
            object: targetTab
        )
        showFeedback("Dropping down Settings ⚙️ (\(targetTab.rawValue))")
    }

    @ViewBuilder
    private var smartSettingsDropdownStripView: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Image(systemName: "sparkles")
                    .font(.system(size: 10, weight: .bold))
                    .foregroundColor(.yellow)
                Text("GENIE SMART SETTINGS")
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
                    ForEach(matchingSettingsTabs.prefix(6)) { tab in
                        Button(action: {
                            openSmartSettings(for: tab.rawValue)
                        }) {
                            HStack(spacing: 5) {
                            ...
                            .background(
                                Capsule()
                                    .fill(tab.tintColor.opacity(0.20))
                                    .overlay(
                                        Capsule()
                                            .strokeBorder(tab.tintColor.opacity(0.45), lineWidth: 0.75)
                                    )
                            )
                        }
                        .buttonStyle(.plain)
    ...
    private func executeSearch() {
        ...
        // 1. Enable web access & real-time search citations
        localModels.webAccessEnabled = true

        // 2. Synthesize AI search with top reference links in Forever Chat
        let searchPrompt = "Search the web and provide a clear, accurate, and comprehensive answer for: \"\(query)\". Include the top best reference links, official sources, and web citations formatted as clean clickable markdown links (e.g. [Source Title](https://...)) so they can be opened in the browser."

        // 3. Switch seamlessly to Forever Chat
        withAnimation(.spring(response: 0.30, dampingFraction: 0.82)) {
            currentMode = .chat
            barModeRaw = "chat"
        }

        localModels.generate(prompt: searchPrompt)

        withAnimation(.easeOut(duration: 0.15)) {
            noteText = ""
        }
        showFeedback("AI Searching in Forever Chat 🌐✨")
    }
    ...
    private func showFeedback(_ msg: String) {
        ...
        feedbackTimer = Timer.scheduledTimer(withTimeInterval: 1.8, repeats: false) { _ in
            Task { @MainActor in
    ...
    private var shouldShowChatStream: Bool {
        !localModels.chatHistory.isEmpty || localModels.isGenerating || !localModels.currentResponse.isEmpty || !noteText.isEmpty || isFocused
    }
    ...
    private var verticalFadingChatStreamView: some View {
        VStack(spacing: 0) {
            ...
            // ── TOP SEARCH INPUT BAR ("Search, ask Genie, or enter web URL...") ──
            HStack(spacing: 8) {
                Image(systemName: "magnifyingglass")
                ...
                NativeSearchTextField(
                    text: $noteText,
                    placeholder: "Search, ask Genie, or enter web URL...",
                    onSubmit: { handleSubmit() },
                    onCommandSubmit: { handleCommandSubmit() },
                    onEscape: { handleEscape() },
                    onFocusChanged: { focused in isFocused = focused }
                )
            ...
            // ── MODE PILLS ROW: [💬 Chat] [🌐 Browser] [📄 Notes] [💻 Terminal] [⤢ Drop to Space] ──
            HStack(spacing: 6) {
                // Chat Pill
                Button(action: {
                    withAnimation(.spring(response: 0.28, dampingFraction: 0.82)) { currentMode = .chat }
                    ...
                    .background(
                        Capsule()
                            .fill(currentMode == .chat ? Color.cyan.opacity(0.38) : Color.white.opacity(0.08))
                    )
                    .overlay(
                        Capsule()
                            .stroke(currentMode == .chat ? Color.cyan.opacity(0.70) : Color.white.opacity(0.12), lineWidth: 0.75)
                    )
                }
                .buttonStyle(.plain)
                ...
                Button(action: {
                    ...
                    HapticFeedback.heavy()
                    showFeedback(isChatFullScreen ? "Dropped to Space ⛶" : "Window Restored")
                }) {
                    HStack(spacing: 5) {
            ...
            // ── CENTERED HISTORY PILL: [🕒 Load Earlier Chats (22 archived)] ──
            let archivedCount = max(22, localModels.chatHistory.count + 12)
            Button(action: {
                showSavedChatsDrawer.toggle()
                HapticFeedback.selection()
                showFeedback("Loaded Previous Turns 🕒")
            }) {
                HStack(spacing: 6) {
                    Image(systemName: "clock.arrow.circlepath")
                        .font(.system(size: 10.5, weight: .bold))
                        .foregroundColor(.cyan)
                    Text("Load Earlier Chats (\(archivedCount) archived)")
                        .font(.system(size: 11, weight: .semibold, design: .rounded))
                        .foregroundColor(.cyan)
                }
                .padding(.horizontal, 16)
                ...
                .overlay(
                    Capsule()
                        .stroke(Color.cyan.opacity(0.35), lineWidth: 0.75)
                )
            }
            .buttonStyle(.plain)
            .padding(.top, 10)
            ...
            // ── SCROLLABLE FOREVER CHAT STREAM (Most recent chat at bottom, infinite recall upwards) ──
            ScrollViewReader { proxy in
                ...
                .overlay(alignment: .bottomTrailing) {
                    if localModels.chatHistory.count > 3 {
                        Button(action: {
                        ...
                        .padding(.trailing, 12)
                        .padding(.bottom, 6)
                        .help("Jump to Latest Chat Bubble")
                    }
                }
            ...
            // ── BOTTOM STATUS BAR: • Model Cycler (Forever Chat) • 19 turns | New Chat ──
            chatStreamBottomStatusBar
        }
        ...
        .background(
            ZStack {
                ...
                LinearGradient(
                    stops: [
                        .init(color: Color(red: 0.12, green: 0.13, blue: 0.20).opacity(0.92), location: 0.0),
                        .init(color: Color(red: 0.07, green: 0.08, blue: 0.13).opacity(0.95), location: 0.5),
                        .init(color: Color(red: 0.04, green: 0.05, blue: 0.09).opacity(0.97), location: 1.0)
                    ],
                    startPoint: .top,
                    endPoint: .bottom
                )

                // Specular top lighting glaze
                LinearGradient(
                ...
                .strokeBorder(
                    LinearGradient(
                        stops: [
                            ...
                            .init(color: Color.cyan.opacity(0.30), location: 0.25),
    ...
    // MARK: - Chat Stream Bottom Status Bar
    @ViewBuilder
    private var chatStreamBottomStatusBar: some View {
        let statusModelName = localModels.selectedModelDisplayName
        let turnCount = max(1, localModels.chatHistory.count)
        let statusText = "\(statusModelName) • \(turnCount) turns"
        let badgeColor = localModels.activeProvider.badgeColor

        HStack {
            Button(action: {
                HapticFeedback.selection()
                _ = localModels.cycleNextModel()
                showFeedback("Switched to \(statusModelName) 🧠✨")
            }) {
                HStack(spacing: 6) {
                    Circle()
                        .fill(badgeColor)
            ...
            .buttonStyle(.plain)
            .help("Click to cycle AI Models • Right-click for Model Menu")
            .contextMenu {
                modelSelectionContextMenu
            }
    ...
    @ViewBuilder
    private var modelSelectionContextMenu: some View {
        Section("Google Gemini") {
            Button("Gemini 2.0 Flash (Fast)") {
                localModels.selectModel("gemini-2.0-flash")
                showFeedback("Switched to Gemini 2.0 Flash ⚡️")
            }
            Button("Gemini 1.5 Pro (Deep)") {
                localModels.selectModel("gemini-1.5-pro")
                showFeedback("Switched to Gemini 1.5 Pro 🧠")
            }
        }
        Section("Anthropic Claude") {
            Button("Claude 3.7 Sonnet (Hybrid)") {
                localModels.selectModel("claude-3-7-sonnet-20250219")
                showFeedback("Switched to Claude 3.7 Sonnet 🔮")
            }
            Button("Claude 3.5 Sonnet (Code)") {
                localModels.selectModel("claude-3-5-sonnet-20241022")
                showFeedback("Switched to Claude 3.5 Sonnet 💻")
            }
            Button("Claude 3.5 Haiku (Fast)") {
                localModels.selectModel("claude-3-5-haiku-20241022")
                showFeedback("Switched to Claude 3.5 Haiku ⚡️")
            }
        }
        Section("OpenAI") {
            Button("GPT-4o (Omni)") {
                localModels.selectModel("gpt-4o")
                showFeedback("Switched to GPT-4o 🌟")
            }
            Button("o3-mini (Reasoning)") {
                localModels.selectModel("o3-mini")
                showFeedback("Switched to o3-mini 🧩")
            }
        }
        Section("Local / Offline AI") {
            Button("Genie Nano 50MB (Neural Super-Res)") {
                localModels.selectModel("genie-nano-50mb")
                showFeedback("Switched to Genie Nano 50MB 🚀")
            }
            Button("AGY (Antigravity Agent)") {
                localModels.selectModel("agy")
                showFeedback("Switched to AGY Agent 🤖")
            }
            Button("Gemma 2 (Local Weights)") {
                localModels.selectModel("gemma-2")
                showFeedback("Switched to Gemma 2 🏠")
            }
        }
        Divider()
        Button("Auto-Select Best Model") {
            localModels.enableAutoSelect()
            showFeedback("Auto-select enabled ✨")
        }
    }
        ...
        HStack(alignment: .bottom, spacing: 8) {
            ...
            VStack(alignment: isUser ? .trailing : .leading, spacing: 4) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    // 5. Inline Web Link Citations & Reference Pills
                    if !isUser {
                        let webLinks = localModels.extractWebLinks(from: msg.content)
                        if !webLinks.isEmpty {
                            VStack(alignment: .leading, spacing: 5) {
                                HStack(spacing: 4) {
                                    ...
                                    Text("Top Sources & Reference Links:")
                                        .font(.system(size: 9.5, weight: .semibold))
                                        .foregroundColor(.white.opacity(0.65))
                                }
                                .padding(.top, 3)
                                ...
                                ForEach(webLinks) { link in
                                    InlineWebLinkCitationView(link: link)
                                }
                            }
                            .padding(.top, 4)
                        }
                ...
                let isShortUserMsg = isUser && msg.content.trimmingCharacters(in: .whitespacesAndNewlines).count <= 8 && !msg.content.contains("\n")

                VStack(alignment: .leading, spacing: 6) {
                    Text(msg.content)
                        .font(.system(size: 13, weight: isShortUserMsg ? .bold : .regular, design: .rounded))
                        .foregroundColor(.white)
                        .textSelection(.enabled)
                }
                .padding(.horizontal, isShortUserMsg ? 16 : 14)
                .padding(.vertical, isShortUserMsg ? 8 : 10)
                .background(
                    ZStack {
                        if isUser {
                            LinearGradient(
                                colors: [
                                    Color(red: 0.05, green: 0.58, blue: 0.98),
                                    Color(red: 0.12, green: 0.72, blue: 1.0)
                                ],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing
                            )
                            .clipShape(isShortUserMsg ? AnyShape(Capsule()) : AnyShape(RoundedRectangle(cornerRadius: 16, style: .continuous)))
                        } else {
                            VisualEffectBlur(material: .popover, blendingMode: .withinWindow, state: .active)
                                .clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
                        ...
                        if isUser {
                            if isShortUserMsg {
                                Capsule()
                                    .strokeBorder(
                                        LinearGradient(
                                            colors: [Color.white.opacity(0.60), Color.white.opacity(0.15)],
                                            startPoint: .top,
                                            endPoint: .bottom
                                        ),
                                        lineWidth: 0.8
                                ...
                                RoundedRectangle(cornerRadius: 16, style: .continuous)
                                    .strokeBorder(
                                        LinearGradient(
                                            colors: [Color.white.opacity(0.50), Color.cyan.opacity(0.60), Color.white.opacity(0.15)],
                                            startPoint: .topLeading,
                                            endPoint: .bottomTrailing
                                        ),
                                        lineWidth: 0.85
                            ...
                            RoundedRectangle(cornerRadius: 16, style: .continuous)
                                .strokeBorder(
                                    LinearGradient(
                                        colors: [Color.white.opacity(0.28), Color.white.opacity(0.08), Color.cyan.opacity(0.18)],
                                        startPoint: .topLeading,
                                        endPoint: .bottomTrailing
                                    ),
                                    lineWidth: 0.85
                ...
                .shadow(
                    color: isUser ? Color.blue.opacity(0.40) : Color.black.opacity(0.35),
                    radius: isUser ? 8 : 12,
                    x: 0,
                    y: isUser ? 2 : 5
                )
