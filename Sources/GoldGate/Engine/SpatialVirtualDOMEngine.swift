...
public final class SpatialVirtualDOMEngine: ObservableObject {
    ...
    // ── Build Virtual DOM Tile Nodes (Zero Padding 9x9 = 81 Big Screen Universe) ──
    public func buildInitialVirtualTree(screenWidth: CGFloat, screenHeight: CGFloat) {
        let is81 = SpatialPlaneManager.shared.isUniverse81Active
        let dim = is81 ? 9 : 3
        let totalCount = dim * dim

        self.viewportSize = CGSize(width: screenWidth, height: screenHeight)
        self.canvasSize = CGSize(width: screenWidth * CGFloat(dim), height: screenHeight * CGFloat(dim))
    ...
    public func updateViewport(cameraOffset: CGSize, viewportSize: CGSize, focusedSlot: Int) {
        ...
        for i in 0..<virtualTiles.count {
            ...
            // Realized Graphics Mode: all 9 desktop sector tiles render 100% realized live graphics
            virtualTiles[i].isVisible = true
