...
public struct AppleSearchBarNoteView: View {
    ...
    private func chatBarSpacePill(space: MacDesktopSpace) -> some View {
        ...
        let textColor: Color = isCurr ? .cyan : Color.white.opacity(0.65)
        let bgColor: Color = isCurr ? Color.cyan.opacity(0.30) : Color.white.opacity(0.08)
        let strokeColor: Color = isCurr ? Color.cyan.opacity(0.65) : Color.white.opacity(0.12)

        Button(action: {
            HapticFeedback.selection()
            MacDesktopsManager.shared.switchToSpace(index: space.index)
        }) {
            HStack(spacing: 2) {
    ...
    private func chatBarDockAppIcon(item: DockAppItem) -> some View {
        Button(action: {
            ...
            ZStack(alignment: .bottom) {
                ...
                if item.isRunning {
                    Circle()
                        .fill(item.isActive ? Color.cyan : Color.white.opacity(0.85))
