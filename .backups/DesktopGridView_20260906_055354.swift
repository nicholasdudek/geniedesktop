...
@ObservedObject var appModel: AppModel
    ...
    @AppStorage("nexus.appDisplayStage") var appDisplayStageRaw: Int = 0 // Default: .hidden (0)
    ...
    var appDisplayStage: AppDisplayStage {
        ...
        nonmutating set { appDisplayStageRaw = newValue.rawValue }
    }

    // Appearance & Layout Shape Settings
    @AppStorage("nexus.showAppNames") var showAppNames: Bool = false
    @AppStorage("nexus.showPageIndicator") var showPageIndicator: Bool = true
    @AppStorage("nexus.iconSize") var iconSize: Double = 64.0
    @AppStorage("nexus.textSize") var textSize: Double = 11.0
    @AppStorage("nexus.spacing") var itemSpacing: Double = 16.0
    @AppStorage("nexus.enableMagnification") var enableMagnification: Bool = true
    @AppStorage("nexus.magnificationScale") var maxMagnification: Double = 1.65
    @AppStorage("nexus.wallpaperMode") var wallpaperMode: String = "Genie"
    @AppStorage("nexus.sameWallpaperMode") var sameWallpaperMode: Bool = true
    @AppStorage("nexus.wallpaperMatchingStyle") var wallpaperMatchingStyle: String = "Exact Mirror (1:1)"
    @AppStorage("nexus.wallpaperTreatment") var wallpaperTreatment: String = "Exact Mirror (1:1)"
    @AppStorage("nexus.appIconTheme") var appIconTheme: String = "Default"
    @AppStorage("nexus.alwaysOnDesktop") var alwaysOnDesktop: Bool = false
    @AppStorage("nexus.appIconTintColor") var appIconTintColor: String = "Emerald"
    @AppStorage("nexus.iconSnuggie") var iconSnuggie: String = "None"
    
    // Curated Selection-Guaranteed Shapes & Living Ambient Entities
    @AppStorage("nexus.appFormation") var appFormation: String = "Responsive Grid"
    @AppStorage("nexus.chatGridPadding") var chatGridPadding: Double = 48.0
    @AppStorage("nexus.ambientEntity") var ambientEntity: String = "None"
    @AppStorage("nexus.dragonFollowCursor") var dragonFollowCursor: Bool = true
    @State private var desktopScrollOffsetY: CGFloat = 0.0

    // Dynamic Wallpaper Effects Engine
    @AppStorage("nexus.wallpaperFxEnabled") var wallpaperFxEnabled: Bool = false
    @AppStorage("nexus.wallpaperFxType") var wallpaperFxType: String = "Cosmic Aurora"
    @AppStorage("nexus.wallpaperFxIntensity") var wallpaperFxIntensity: Double = 0.35

    // Kinetic & Orbit Dynamics
    @AppStorage("nexus.orbitSpeed") var orbitSpeed: Double = 1.0
    @AppStorage("nexus.orbitClockwise") var orbitClockwise: Bool = true
    @AppStorage("nexus.pulseIntensity") var pulseIntensity: Double = 0.5

    // Living Background App Physics Simulation
    @AppStorage("nexus.appPhysicsSimulation") var appPhysicsSimulation: String = "None"

    // Cursor FX Animation Engine
    @AppStorage("nexus.cursorFxType") var cursorFxType: String = "None"
    @AppStorage("nexus.smokeStyle") var smokeStyle: String = "Mystical Cyan 🧞‍♂️"

    // Page 1 vs Page 2 Default Behavior
    @AppStorage("nexus.defaultPageMode") var defaultPageMode: String = "Page 1 (Apps Direct)"
    @AppStorage("nexus.desktopSplitMode") var desktopSplitMode: String = "Full Screen"
    @AppStorage("nexus.gridTransitionDirection") var gridTransitionDirection: String = "Pull Up from Bottom"
    ...
    // Right-Side Sliding Docks (Independent Chat & Applications) - Consolidated into Chat Bar
    @AppStorage("nexus.rightEdgeDocksEnabled") var rightEdgeDocksEnabled: Bool = false
    @AppStorage("nexus.isRightChatDockOpen") var isRightChatDockOpen: Bool = false
    @AppStorage("nexus.isRightAppsDockOpen") var isRightAppsDockOpen: Bool = false
    @AppStorage("nexus.rightDocksCoexistMode") var rightDocksCoexistMode: String = "Side-by-Side 📐"
    @State private var isTopSearchBarPoppedDown: Bool = false
    @State private var isSearchBarTyping: Bool = false
    @AppStorage("nexus.bottomEdgeCursorTrigger") var bottomEdgeCursorTrigger: Bool = true
    @AppStorage("nexus.topEdgeCursorTrigger") var topEdgeCursorTrigger: Bool = true
    @AppStorage("nexus.swipeSensitivity") var swipeSensitivity: String = "Deliberate (Firm Swipe)"
    @AppStorage("nexus.swipeTriggerZone") var swipeTriggerZone: String = "Both (Top & Bottom)"
    @AppStorage("nexus.dockAvoidanceEnabled") var dockAvoidanceEnabled: Bool = true
    @AppStorage("nexus.dockRestPeriod") var dockRestPeriod: Double = 0.65

    // Arcade Pinball Mode & Pin to Desktop
    @AppStorage("nexus.pinballModeEnabled") var pinballModeEnabled: Bool = false
    @AppStorage("nexus.pinToDesktopEnabled") var pinToDesktopEnabled: Bool = false
    @AppStorage("nexus.formationAppLimit") var formationAppLimit: Int = 0
    @AppStorage("nexus.customFormationColumns") var customFormationColumns: Int = 4
    ...
    var body: some View {
        GeometryReader { screenGeo in
            ...
            let topClearance = max(36.0, targetScreen.frame.maxY - targetScreen.visibleFrame.maxY + 18.0)
            ...
            ZStack(alignment: .top) {
                ...
                Color.clear
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 12)
                            .onChanged { value in
                                guard !isEditing && currentPage == 1 else { return }
                                let isHorizontal = (gridTransitionDirection == "Slide from Right (iPhone Mode 📱)" || gridTransitionDirection == "Slide from Left (Sidebar ⬅️)")
                                if isHorizontal {
                                    pullDragOffsetX = value.translation.width * 0.75
                                    pullDragOffsetY = 0
                                } else {
                                    pullDragOffsetX = 0
                                    pullDragOffsetY = value.translation.height * 0.75
                                }
                            }
                            .onEnded { value in
                                guard !isEditing && currentPage == 1 else { return }
                                ...
                                case "Drop Down from Top (Menu Bar ⬇️)":
                                    shouldDismiss = value.translation.height < -60 || value.predictedEndTranslation.height < -100
                                default: // Pull Up from Bottom
                                    shouldDismiss = value.translation.height > 60 || value.predictedEndTranslation.height > 100
                                }

                                if !pinToDesktopEnabled && !alwaysOnDesktop && shouldDismiss {
                                    HapticFeedback.tick()
                ...
                .offset(
                    x: transitionOffset(screenSize: screenSize).x,
                    y: transitionOffset(screenSize: screenSize).y + (
                        (searchBarPlacement == "Meet in Middle (Top Chat, Bottom Apps) ⚖️" && !appFormation.contains("Responsive") && !appFormation.contains("Surround"))
                            ? (isSearchBarTyping ? screenSize.height * 0.55 : screenSize.height * middleSplitRatio)
                            : (appDisplayStage == .middle ? screenSize.height * 0.22 : 0)
                    )
                )
                .scaleEffect(transitionOffset(screenSize: screenSize).scale * (isSearchBarTyping ? 0.94 : 1.0))
                .opacity((currentPage == 1 && appDisplayStage != .hidden) ? (isSearchBarTyping ? 0.08 : 1.0) : 0.0)
                .allowsHitTesting(currentPage == 1 && appDisplayStage != .hidden && !isSearchBarTyping)
                .animation(.spring(response: 0.44, dampingFraction: 0.88), value: currentPage)
                .animation(.spring(response: 0.44, dampingFraction: 0.88), value: isSearchBarTyping)
                .animation(.spring(response: 0.44, dampingFraction: 0.88), value: appDisplayStageRaw)

                // Top Floating Overlays & Controls
                topFloatingOverlays(
                    screenSize: screenSize,
                    usableWidth: usableWidth,
                    usableHeight: usableHeight,
                ...
                case .active(let location):
                    ...
                    if !isEditing {
                        ...
                        if now - lastPageSwitchTime > 0.25 {
                            // 1. Mouse down to bottom edge: brings apps up!
                            if (currentPage == 0 || appDisplayStage == .hidden) && location.y >= screenSize.height - 24 && bottomEdgeCursorTrigger {
                            ...
                            // 2. Top edge: brings chat with wallpaper!
                            else if (currentPage == 0 || appDisplayStage != .hidden || !isTopSearchBarPoppedDown) && location.y <= 6 && topEdgeCursorTrigger {
                                lastPageSwitchTime = now
                                HapticFeedback.selection()
                                withAnimation(.spring(response: 0.44, dampingFraction: 0.88)) {
                                    currentPage = 1
                            ...
                            // 3. Mouse up to hide: when apps are up, moving mouse up past the top boundary can hide
                            else if currentPage == 1 && appDisplayStage == .fullScreen && location.y <= 4 && !isSearchBarTyping && !pinToDesktopEnabled && !alwaysOnDesktop {
                                lastPageSwitchTime = now
                                HapticFeedback.tick()
                                withAnimation(.spring(response: 0.44, dampingFraction: 0.88)) {
                                    currentPage = 0
                            ...
                            // 4. Mouse down to hide: when chat is up, moving mouse down towards bottom edge can hide
            ...
            .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("NexusToggleRightChatDock"))) { _ in
                withAnimation(.spring(response: 0.32, dampingFraction: 0.82)) {
                    if rightDocksCoexistMode == "Auto-Swap 🔄" && !isRightChatDockOpen {
            ...
            .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("NexusToggleRightAppsDock"))) { _ in
                withAnimation(.spring(response: 0.32, dampingFraction: 0.82)) {
                    if rightDocksCoexistMode == "Auto-Swap 🔄" && !isRightAppsDockOpen {
                        isRightChatDockOpen = false
                    }
                    isRightAppsDockOpen.toggle()
                }
            }
            ...
            .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("NexusTopEdgeHit"))) { _ in
                ...
                HapticFeedback.selection()
                withAnimation(.spring(response: 0.32, dampingFraction: 0.82)) {
                    if currentPage == 0 || appDisplayStage != .hidden || !isTopSearchBarPoppedDown {
                        currentPage = 1
                        appDisplayStage = .hidden
                        isTopSearchBarPoppedDown = true
                        DesktopWindowManager.shared.setPage(1)
        ...
        .onChange(of: appDisplayStageRaw) { _, _ in
        ...
        .onChange(of: isTopSearchBarPoppedDown) { _, _ in
            DesktopWindowManager.shared.syncCurrentStation(fromPage: currentPage, appDisplayStage: appDisplayStage, isTopSearchBarPoppedDown: isTopSearchBarPoppedDown)
        }
        ...
        .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("NexusRevealSearchBarForTyping"))) { _ in
            withAnimation(.spring(response: 0.32, dampingFraction: 0.82)) {
                ...
                if searchBarPlacement.contains("Top") {
                    isTopSearchBarPoppedDown = true
                }
            }
        }
    ...
    private func continuousVerticalScrollBar(screenSize: CGSize) -> some View {
        let currentStation: Int = {
            if currentPage == 1 && appDisplayStage == .hidden && isTopSearchBarPoppedDown {
                return 0 // Zenith (Top: Chat)
            } else if currentPage == 1 && appDisplayStage == .fullScreen {
                return 2 // Nadir (Bottom: Apps)
            } else {
                return 1 // Horizon (Center: Desktop)
            }
        }()
        ...
        HStack {
            ...
            VStack(spacing: 8) {
                ...
                Button(action: {
                    ...
                    ZStack {
                        Circle()
                            .fill(currentStation == 0 ? Color.white.opacity(0.30) : Color.white.opacity(0.06))
                            .frame(width: 22, height: 22)
                        Image(systemName: "bubble.left.and.bubble.right.fill")
                            .font(.system(size: 9.5, weight: .medium))
                            .foregroundColor(currentStation == 0 ? .white : .white.opacity(0.45))
                    }
                }
                .buttonStyle(.plain)
                .help("Zenith — Dialogue Studio (Top Edge)")

                // Horizon: Clean Desktop Canvas
                Button(action: {
                    HapticFeedback.selection()
                    ...
                    ZStack {
                        Circle()
                            .fill(currentStation == 1 ? Color.white.opacity(0.30) : Color.white.opacity(0.06))
                            .frame(width: 22, height: 22)
                        Image(systemName: "macwindow")
                            .font(.system(size: 9.5, weight: .medium))
                            .foregroundColor(currentStation == 1 ? .white : .white.opacity(0.45))
                    }
                }
                .buttonStyle(.plain)
                .help("Horizon — Architectural Canvas (Center)")

                // Nadir: Application Atelier
                Button(action: {
                    HapticFeedback.selection()
                    ...
                    ZStack {
                        Circle()
                            .fill(currentStation == 2 ? Color.white.opacity(0.30) : Color.white.opacity(0.06))
                            .frame(width: 22, height: 22)
                        Image(systemName: "square.grid.2x2.fill")
                            .font(.system(size: 9.5, weight: .medium))
                            .foregroundColor(currentStation == 2 ? .white : .white.opacity(0.45))
                    }
                }
                .buttonStyle(.plain)
                .help("Nadir — Application Atelier (Bottom Edge)")
            }
            .padding(.vertical, 8)
    ...
    private func topFloatingOverlays(screenSize: CGSize, usableWidth: CGFloat, usableHeight: CGFloat, topClearance: CGFloat, bottomClearance: CGFloat, sideMargin: CGFloat) -> some View {
        ...
        if (currentPage == 1 || isTopSearchBarPoppedDown) && !isEditing {
            ...
            let customPadding = CGFloat(UserDefaults.standard.double(forKey: "nexus.chatGridPadding"))
            let chatPad = (customPadding > 0) ? customPadding : 48.0
            let searchBarTop: CGFloat = {
                if appFormation == "Lined Up Above Chat ⬆️" || appFormation.contains("Above Chat") {
                    let spacing = max(14.0, CGFloat(itemSpacing))
                    let cellW = CGFloat(iconSize) + spacing
                    let maxCols = max(3, Int(floor((usableWidth - 40.0) / cellW)))
                    let idealCols = min(maxCols, max(4, Int(ceil(sqrt(Double(apps.count) * 1.8)))))
                    let cols = max(1, idealCols)
                    let rows = max(1, Int(ceil(Double(apps.count) / Double(cols))))
                    let gridH = CGFloat(rows) * (CGFloat(iconSize) + nameH) + CGFloat(max(0, rows - 1)) * spacing
                    let searchBarH: CGFloat = 50.0
                    let totalStackH = gridH + chatPad + searchBarH
                    let startY = max(topClearance + 24.0, topClearance + max(16.0, (usableHeight - totalStackH) * 0.38))
                    return startY + gridH + chatPad
                } else if (appFormation == "Responsive Grid" || appFormation.contains("Surround")) {
                    return max(topClearance + 16, metrics.searchBarCenterY - 24.0)
                } else {
                    return max(topClearance + 44, screenSize.height * 0.13)
                }
            }()
            ...
            VStack(spacing: 0) {
                if searchBarPlacement == "Meet in Middle (Top Chat, Bottom Apps) ⚖️" {
                    if appDisplayStage == .hidden {
                        AppleSearchBarNoteView()
                            .padding(.top, topClearance + 12.0)
                            .frame(maxHeight: isSearchBarTyping ? screenSize.height * 0.82 : screenSize.height * middleSplitRatio, alignment: .top)
                    ...
                    Spacer()
                    if currentPage == 1 && !isEditing && appDisplayStage == .hidden {
                        appDrawerStageHandle(screenSize: screenSize)
                            .padding(.bottom, bottomClearance + 14.0)
                    }
                } else if isTopSearchBarPoppedDown || searchBarPlacement == "Top Middle Pop-Down ⬆️" {
                    if appDisplayStage == .hidden || isTopSearchBarPoppedDown {
                        AppleSearchBarNoteView()
                            .padding(.top, topClearance + 12.0)
                            .transition(.asymmetric(
                                insertion: .move(edge: .top).combined(with: .opacity),
                ...
                } else if searchBarPlacement == "Centered Dynamic 🎯" && appFormation != "Lined Up Above Chat ⬆️" {
                    if appDisplayStage == .hidden {
                        AppleSearchBarNoteView()
                            .padding(.top, searchBarTop)
                    ...
                    // Default: "Bottom Above Dock ⬇️"
                    Spacer()

                    // Applications Elevation Stage Handle Pill
                    if currentPage == 1 && !isEditing && appDisplayStage == .hidden {
                        appDrawerStageHandle(screenSize: screenSize)
                            .padding(.bottom, 8)
                    }
                    ...
                    if appDisplayStage == .hidden {
                        AppleSearchBarNoteView()
                            .padding(.bottom, bottomClearance + 18.0)
                            .transition(.asymmetric(
        ...
        if showPageIndicator && !isEditing {
    ...
    // MARK: - 📱 Right-Edge Sliding Docks & Trigger Tabs (Chat & Applications)
    @ViewBuilder
    private func rightEdgeDocksContainer(screenSize: CGSize, topClearance: CGFloat, bottomClearance: CGFloat) -> some View {
        ...
        ZStack(alignment: .trailing) {
            // Sliding Docks Container
            HStack(spacing: 8) {
                // If both are open and mode is "Side-by-Side 📐", Chat Dock sits on the inside (left)
                if isRightChatDockOpen && (isRightAppsDockOpen && rightDocksCoexistMode == "Side-by-Side 📐") {
                ...
                // Applications Dock
                if isRightAppsDockOpen {
                    RightSideApplicationsDockView(
                        isRightAppsDockOpen: $isRightAppsDockOpen,
                        isRightChatDockOpen: $isRightChatDockOpen
                    )
                    .frame(height: usableHeight)
                ...
                // Chat Dock (when Apps is not open, or when not side-by-side)
                if isRightChatDockOpen && !(isRightAppsDockOpen && rightDocksCoexistMode == "Side-by-Side 📐") {
                    RightSideChatDockView(
                        isRightChatDockOpen: $isRightChatDockOpen,
                        isRightAppsDockOpen: $isRightAppsDockOpen
            ...
            // Right-Edge Floating Trigger Tabs
            RightEdgeDockTabsView(
                isRightChatDockOpen: $isRightChatDockOpen,
                isRightAppsDockOpen: $isRightAppsDockOpen,
                coexistMode: rightDocksCoexistMode
            )
            .position(x: screenSize.width - 16, y: screenSize.height * 0.48)
        }
    ...
    // MARK: - Applications Elevation Handle (Shown only when applications are hidden)
    @ViewBuilder
    ...
    private func updateCursorTrail(_ point: CGPoint) {
        ...
        if cursorTrail.count > 64 {
            cursorTrail.removeFirst(cursorTrail.count - 64)
        }
    }
    ...
    private func handleScrollEvent(_ event: NSEvent) {
        ...
        // Anti-jitter cooldown between page flips
        guard now - lastPageSwitchTime > 0.22 else { return }
        ...
        // Forward if Desktop 2 is active, or Finder/Genie is front, or mouse is over desktop
        ...
        // 3. Process trackpad swipe and mouse scroll events with fluid, natural thresholds
        let deltaY = event.scrollingDeltaY
        let deltaX = event.scrollingDeltaX
        let isHorizontalMode = (gridTransitionDirection == "Slide from Right (iPhone Mode 📱)" || gridTransitionDirection == "Slide from Left (Sidebar ⬅️)")

        let flickThreshold: CGFloat = 14.0
        let longPullThreshold: CGFloat = 26.0

        if currentPage == 0 {
            // Summon intent on Clean Desktop:
            let primaryDelta = isHorizontalMode ? deltaX : deltaY

            if now - lastScrollTime > 0.40 || (scrollAccumulator > 0 && primaryDelta < 0) || (scrollAccumulator < 0 && primaryDelta > 0) {
                scrollAccumulator = 0
            }
            lastScrollTime = now
            scrollAccumulator += primaryDelta

            let isNegativePull = (primaryDelta <= -flickThreshold || scrollAccumulator <= -longPullThreshold)
            let isPositivePull = (primaryDelta >= flickThreshold || scrollAccumulator >= longPullThreshold)

            var shouldSummon = false
            let allowScrollSummon = UserDefaults.standard.bool(forKey: "nexus.trackpadScrollSummonEnabled")
            if allowScrollSummon {
                switch gridTransitionDirection {
                case "Slide from Right (iPhone Mode 📱)":
                    shouldSummon = isNegativePull // Swiping leftward summons from right
                case "Slide from Left (Sidebar ⬅️)":
                    shouldSummon = isPositivePull // Swiping rightward summons from left
                case "Drop Down from Top (Menu Bar ⬇️)":
                    shouldSummon = isPositivePull // Swiping downward summons from top
                default: // "Pull Up from Bottom" and "Spatial Zoom"
                    shouldSummon = isNegativePull // Swiping upward summons from bottom
                }
            }
            ...
            // On Desktop 2 (Applications Canvas):
            // Check if user is performing an intentional swipe-to-dismiss
            if isHorizontalMode {
                // In horizontal slide mode, ONLY a deliberate horizontal swipe can dismiss!
                // Vertical scrolling (deltaY) NEVER dismisses the grid.
                let isDismissSwipe: Bool
                switch gridTransitionDirection {
                case "Slide from Right (iPhone Mode 📱)":
                    isDismissSwipe = deltaX >= flickThreshold // Swiping rightward dismisses back
                case "Slide from Left (Sidebar ⬅️)":
                    isDismissSwipe = deltaX <= -flickThreshold // Swiping leftward dismisses back
                default:
                    isDismissSwipe = false
                }

                if isDismissSwipe && !alwaysOnDesktop && !pinToDesktopEnabled {
                    scrollAccumulator = 0
                    lastPageSwitchTime = now
                    desktopScrollOffsetY = 0
                    HapticFeedback.tick()
            ...
            // Route vertical scrolling to the apps formation matrix
            let rawDelta = event.scrollingDeltaY
            let multiplier: CGFloat = event.hasPreciseScrollingDeltas ? 1.0 : 8.0
            let vDelta = rawDelta * multiplier

            let curScreen = screen ?? NSScreen.main ?? (NSScreen.screens.first ?? NSScreen())
            let curUsableHeight = max(200.0, curScreen.frame.height - 120.0)
            let curUsableWidth = max(300.0, curScreen.frame.width - 80.0)
            let curSpacing = CGFloat(itemSpacing)
            let curIconSize = CGFloat(iconSize)
            let curTextSize = CGFloat(textSize)

            let cols: Int
            if appFormation == "Classic 3x3 Compact Grid 📱" {
                cols = 3
            } else if appFormation == "Custom Formation 🛠️" || appFormation == "Custom Formation" {
                cols = max(1, customFormationColumns)
            } else if appFormation == "Dual Column Grid 📑" {
                cols = 2
            } else if appFormation.contains("Train") || appFormation.contains("Rail") {
                cols = 1
            } else {
                cols = max(2, Int(floor(curUsableWidth / (curIconSize + curSpacing))))
            }

            let nameH = showAppNames ? (curTextSize + 8.0) : 0.0
            let rows = max(1, Int(ceil(Double(apps.count) / Double(cols))))
            let totalFormationHeight = CGFloat(rows) * (curIconSize + nameH + curSpacing)
            let minScrollOffset = min(0.0, curUsableHeight - totalFormationHeight - 60.0)

            // For vertical transition modes (Top/Bottom), allow dismiss on trackpad swipe at boundary
            if !isHorizontalMode && !alwaysOnDesktop && !pinToDesktopEnabled {
                ...
                let isAtTop = (desktopScrollOffsetY >= -4.0)
                let isNatural = event.isDirectionInvertedFromDevice
                // Physical finger downward swipe on trackpad:
                // With natural scrolling (macOS default), finger downward swipe yields negative vDelta.
                // Without natural scrolling, finger downward swipe yields positive vDelta.
                let isDownwardPull = isNatural ? (vDelta <= -8.0 || scrollAccumulator <= -12.0) : (vDelta >= 8.0 || scrollAccumulator >= 12.0)
                let isUpwardPull = isNatural ? (vDelta >= 8.0 || scrollAccumulator >= 12.0) : (vDelta <= -8.0 || scrollAccumulator <= -12.0)

                var shouldDismiss = false
                if gridTransitionDirection == "Drop Down from Top (Menu Bar ⬇️)" {
                    if isAtTop && isUpwardPull {
                        shouldDismiss = true
                    }
                } else {
                    // Pull Up from Bottom / standard mode:
                    // Swiping down returns to clean desktop!
                    if (isAtTop || appDisplayStage == .hidden) && isDownwardPull {
                        shouldDismiss = true
                    }
                }
            ...
            let targetOffset = desktopScrollOffsetY + vDelta

            // Smoothly scroll the formation with elastic rubber-banding
            withAnimation(.interactiveSpring(response: 0.20, dampingFraction: 0.82)) {
                desktopScrollOffsetY = max(minScrollOffset - 60.0, min(60.0, targetOffset))
            }

            // Snap back on bounce release
            if event.phase == .ended || event.phase == .cancelled {
                withAnimation(.spring(response: 0.35, dampingFraction: 0.80)) {
                    desktopScrollOffsetY = max(minScrollOffset, min(0.0, desktopScrollOffsetY))
                }
            }
        }
    }
...
let apps: [AppInfo]
        ...
        TimelineView(.animation(minimumInterval: 1.0 / 120.0)) { timeline in
            ...
            ZStack(alignment: .topLeading) {
                ForEach(Array(apps.enumerated()), id: \.element.id) { index, app in
                    ...
                    let center = CGPoint(x: node.center.x + physOffset.width, y: node.center.y + physOffset.height + scrollOffsetY)
    ...
    private func computeFormationLayout(time: Double) -> [String: AppFormationNode] {
        ...
        case "Classic 3x3 Compact Grid 📱":
            // Clean, minimalist iOS-inspired centered 3-column app folder matrix
            let nameH = showName ? (textSize + 8.0) : 0.0
            let cols = 3
            let spacing = max(16.0, curSpacing)
            let totalW = CGFloat(cols) * iconSize + CGFloat(cols - 1) * spacing
            let startX = (screenSize.width - totalW) / 2.0
            let startY = topClearance + 20.0
        ...
        default: // "Responsive Grid" (Full Screen Springboard Matrix surrounding Search Bar)
            ...
            let isChatDetached = UserDefaults.standard.bool(forKey: "nexus.isChatDetached")

            for r in 0..<metrics.rows {
                for c in 0..<metrics.cols {
                    let px = metrics.startX + (iconSize / 2.0) + CGFloat(c) * metrics.cellW
                    // Provide extra vertical breathing buffer for rows above and below the chat bar
                    let yNudge: CGFloat = (r < metrics.searchRow) ? -8.0 : ((r > metrics.searchRow) ? 8.0 : 0.0)
                    let py = metrics.startY + (iconSize / 2.0) + CGFloat(r) * metrics.cellH + yNudge

                    // Check if this cell center overlaps search bar exclusion rect (auto-fill if detached)
                    if !isChatDetached && r == metrics.searchRow && (abs(px - (screenSize.width / 2.0)) < (metrics.searchBarExclusionRect.width / 2.0)) {
...
let icon: NSImage
    ...
    private var shadowColor: Color {
        ...
        case "Gold Gate", "Golden Kintsugi": return Color(red: 1.0, green: 0.75, blue: 0.20).opacity(isHovered ? 0.90 : 0.40)
...
let theme: String
    ...
    func body(content: Content) -> some View {
        ...
        case "Gold Gate", "Golden Kintsugi":
            content
