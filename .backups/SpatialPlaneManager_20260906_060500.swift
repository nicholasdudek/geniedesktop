...
public final class SpatialPlaneManager: ObservableObject {
    ...
    @AppStorage("nexus.screenWebpagePanMode") public var screenWebpagePanMode: String = "Hand Drag & Scroll" // "Hand Drag & Scroll", "Screen Follows Cursor"
    @AppStorage("nexus.cursorFollowPanningEnabled") public var cursorFollowPanningEnabled: Bool = false
    ...
    public func refreshAllRAMBuffers() {
        ...
        for idx in 1...9 {
            ...
            if isCur {
                // Exclude Genie itself from window representations
                let filtered = currentWindows.filter { $0.pid != ProcessInfo.processInfo.processIdentifier }
                buffer.windows = filtered
                buffer.wallpaper = currentWP
                buffer.thumbnail = MacDesktopsManager.shared.desktopLivePreviews[idx] ?? currentWP
                buffer.runningAppNames = Array(Set(filtered.map { $0.ownerName }))
                ...
                // Distinct space identity: NEVER clone our current space capture onto other spaces
                let theme = MacDesktopsManager.curatedThemeForSlot(idx)
                let distinctWP = WallpaperManager.shared.generateCuratedWallpaper(named: theme)
                buffer.wallpaper = distinctWP
                if let preview = MacDesktopsManager.shared.desktopLivePreviews[idx], preview != currentWP {
                    buffer.thumbnail = preview
                } else {
                    buffer.thumbnail = distinctWP
                }
                buffer.windows = []
                buffer.runningAppNames = []
                buffer.runningAppIcons = []
            }
    ...
    public func handleScrollWheel(_ event: NSEvent) {
        ...
        handleThreeFingerScrollDelta(dx: dx, dy: dy)
    }
    ...
    public func navigateGridDirection(_ dir: SpatialPlaneDirection) {
        let (col, row) = Self.gridCoordinate(for: focusedPlaneIndex)
        var newCol = col
        var newRow = row

        switch dir {
        case .east: newCol = min(2, col + 1)
        case .west: newCol = max(0, col - 1)
        case .south: newRow = min(2, row + 1)
        case .north: newRow = max(0, row - 1)
        }

        let newSlot = Self.indexForGrid(col: newCol, row: newRow)
        if newSlot != focusedPlaneIndex {
            focusedPlaneIndex = newSlot
            macroTargetSpaceIndex = newSlot
    ...
    public func settleToNearestSlot() {
...
public final class SpatialTouchHostingView<Content: View>: NSHostingView<Content> {
    ...
    public override func swipe(with event: NSEvent) {
        if SpatialPlaneManager.shared.isZoomedOut {
            if event.deltaX < -0.3 {
                // 3-Finger Swipe Left -> Move East (Right screen)
                SpatialPlaneManager.shared.navigateGridDirection(.east)
            } else if event.deltaX > 0.3 {
                // 3-Finger Swipe Right -> Move West (Left screen)
                SpatialPlaneManager.shared.navigateGridDirection(.west)
            } else if event.deltaY < -0.3 {
                // 3-Finger Swipe Down -> Move North (Top screen)
                SpatialPlaneManager.shared.navigateGridDirection(.north)
            } else if event.deltaY > 0.3 {
                // 3-Finger Swipe Up -> Move South (Bottom screen)
    ...
    public override func touchesEnded(with event: NSEvent) {
        lastThreeFingerPosition = nil
        SpatialPlaneManager.shared.settleToNearestSlot()
